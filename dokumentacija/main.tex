...
\begin{abstract}
U ovom radu predstavljen je informacioni sistem za upravljanje poslovanjem multipleks bioskopa pod nazivom \enquote{Apsolutni bioskop}. Sistem omogućava automatizaciju ključnih poslovnih procesa uključujući online kupovinu i rezervaciju karata, prodaju na blagajni, upravljanje repertoarom i administraciju korisničkih naloga. Rad obuhvata detaljnu analizu zahteva sistema sa identifikacijom četiri glavne grupe korisnika (korisnik, blagajnik, menadžer i sistem administrator) i prototip korisničkog interfejsa koji demonstrira osnovne funkcionalnosti sistema.
\end{abstract}
...
Ovaj rad je nastao kao deo projektnog zadatka na predmetu \enquote{Informacioni sistemi} na master studijama Matematičkog fakulteta. Glavna ideja bila je da teoriju o planiranju sistema primenimo u praksi i pokažemo kako se jedan poslovni proces može detaljno isplanirati.
...
Slika \ref{fig:dijagram_slucajeva_upotrebe} prikazuje sve grupe korisnika i funkcije koje su im dostupne unutar sistema. On služi da se na početku jasno definišu granice sistema i šta svaka uloga može da uradi.
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{../dijagrami/arhitektura_sistema/dijagram_slucajeva_upotrebe.jpg}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{../dijagrami/arhitektura_sistema/dfd_dijagram_konteksta.jpg}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.935\textwidth]{../dijagrami/arhitektura_sistema/dfd_dijagram_nivoa_0.jpg}
...
\input{komponente/online_kupovina_i_registracija.tex}
\newpage
\input{komponente/kupovina_na_blagajni}
\newpage
\input{komponente/registracija}
\newpage
\input{komponente/autorizacija}
\newpage
\input{komponente/upravljanje_podacima}
\newpage
\input{komponente/dodavanje_novih_zaposlenih}
\newpage
\input{komponente/izmena_repertoara}
...
\begin{figure}[h!]
    \centering
    \makebox[\textwidth][c]{
        \includegraphics[width=1.2\textwidth]{../dijagrami/baza_podataka/dijagram_klasa.jpg}
        }
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{../dijagrami/arhitektura_sistema/dijagram_komponenti.jpg}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{../dijagrami/arhitektura_sistema/dijagram_isporucivanja.jpg}
...
Korisnik ulaskom na sajt ApsolutniBioskop vidi početnu stranu sa logom i sloganom "Rezervišite karte za bioskop online". Odavde može kliknuti na dugme \enquote{Pogledaj repertoar} da pregleda dostupne filmove ili \enquote{Prijava} da se uloguje na svoj nalog.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/pocetna_strana.png}
    \caption{Početna stranica}
    \label{fig:pocetna_strana}
\end{figure}

Nakon klika na \enquote{Prijava}, korisnik dolazi na ekran za autentifikaciju gde unosi svoju e-mail adresu i lozinku. Klikom na \enquote{Prijava se} pristupa svom nalogu, a ukoliko nema nalog može kliknuti na \enquote{Registrujte se}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/prijava.png}
    \caption{Prijava}
    \label{fig:prijava}
\end{figure}

\newpage
Nakon prijave, korisnik pregleda trenutni repertoar bioskopa sa prikazanim filmovima (Dina: Drugi deo, Openhajmer, Džon Vik: Četvrti čin). Klikom na željeni film prelazi na stranicu sa detaljima i terminima projekcija.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/repertoar_filmova.png}
    \caption{Repertoar filmova}
    \label{fig:repertoar_filmova}
\end{figure}

Korisnik vidi detalje odabranog filma (opis, režiser, glumci, trajanje) i listu dostupnih termina sa cenama. Klikom na željeni termin (npr. 15.01.2026. u 20:00) prelazi na izbor sedišta u sali.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/prikaz_termina_projekcije.png}
    \caption{Izbor termina projekcije}
    \label{fig:prikaz_termina_projekcije}
\end{figure}

\newpage
Korisnik na interaktivnom prikazu sale bira slobodna sedišta klikom na njih (izabrana sedišta se označavaju crvenom bojom). Nakon izbora vidi ukupnu cenu i može kliknuti \enquote{Rezerviši} za rezervaciju ili \enquote{Kupi} za direktnu kupovinu.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/biranje_sedista.png}
    \caption{Biranje sedista}
    \label{fig:biranje_sedista}
\end{figure}

Klikom na \enquote{Kupi}, korisnik dolazi na stranicu za plaćanje gde vidi pregled porudžbine i može iskoristiti bonus poene za popust. Unosi podatke o platnoj kartici i klikom na \enquote{Plati} završava kupovinu.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/kupovina_karte.png}
    \caption{Kupovina karte}
    \label{fig:kupovina_karte}
\end{figure}

\newpage
Nakon uspešnog plaćanja, korisnik dobija potvrdu sa e-kartom koja sadrži sve detalje (film, termin, sedišta, kod karte). E-karta se šalje na njegovu e-poštu, a prikazuje se i stanje bonus poena.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.88\textwidth]{../gui/potvrda_o_kupovini.png}
    \caption{Potvrda o kupovini}
    \label{fig:potvrda_o_kupovini}
\end{figure}

\newpage
\section{Zaključak}

Informacioni sistem \enquote{Apsolutni bioskop} osmišljen je sa ciljem da modernizuje i pojednostavi poslovanje multipleks bioskopa. U današnje vreme, kada korisnici očekuju brzu i jednostavnu online kupovinu, ovakav sistem predstavlja neophodan korak ka unapređenju korisničkog iskustva i efikasnijem poslovanju.

Tokom izrade ovog projekta, prošli smo kroz sve ključne faze razvoja informacionog sistema, od prikupljanja i analize zahteva, preko modelovanja poslovnih procesa, do projektovanja baze podataka i arhitekture. Korišćenje različitih dijagrama (slučajevi upotrebe, DFD, dijagrami aktivnosti, sekvenci i BPMN) omogućilo nam je da sagledamo sistem iz više uglova i da precizno definišemo kako pojedine komponente međusobno komuniciraju.

Posebna pažnja posvećena je bezbednosti i jasnom razdvajanju uloga jer svaki korisnik sistema ima pristup samo onim funkcionalnostima koje su mu potrebne za obavljanje posla. Ovakav pristup ne samo da štiti osetljive podatke, već i smanjuje mogućnost greške u svakodnevnom radu.

Predložena troslojna arhitektura sa ASP.NET Core okvirom i PostgreSQL bazom podataka pruža solidnu osnovu za buduću implementaciju, dok prototip korisničkog interfejsa demonstrira kako bi krajnji proizvod izgledao iz perspektive korisnika.

\subsection{Dalji razvoj sistema}

U budućnosti se planira proširenje sistema sledećim funkcionalnostima:

\begin{itemize}
    \item \textbf{Mobilna aplikacija} - razvoj nativnih aplikacija za Android i iOS platforme koje bi omogućile još jednostavniju kupovinu karata i pregled repertoara.
    \item \textbf{Sistem preporuka} - implementacija algoritma za personalizovane preporuke filmova na osnovu istorije gledanja i preferencija korisnika.
    \item \textbf{Integracija sa spoljnim servisima} - povezivanje sa sistemima za online plaćanje, servisima za filmske recenzije (npr. IMDb) i društvenim mrežama.
    \item \textbf{Loyalty program} - proširenje sistema bonus poena sa dodatnim beneficijama, popustima i ekskluzivnim ponudama za lojalne korisnike.
    \item \textbf{Analitika i izveštavanje} - napredni modul za generisanje izveštaja o prodaji, posećenosti i trendovima koji bi pomogao menadžmentu u donošenju poslovnih odluka.
\end{itemize}
...
Zahvaljujemo se profesoru i asistentu sa kursa Informacioni sistemi čija su predavanja i vežbe \cite{is_kurs, is_predavanja} bila osnova za izradu ovog projekta. Takođe se zahvaljujemo kolegama čiji su prethodni projekti \cite{is_primeri} poslužili kao korisna referenca i inspiracija.
